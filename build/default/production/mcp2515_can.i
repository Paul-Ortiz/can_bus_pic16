...
unsigned char rtr;
unsigned char mcpMode;

void init_CS(void);
# 50 "./mcp2515_can.h" 2
# 1 "./mcp2515_can_dfs.h" 1
# 53 "./mcp2515_can_dfs.h"
# 1 "E:\\Program Files\\Microchip\\xc8\\v2.45\\pic\\include\\c99\\inttypes.h" 1 3
# 13 "E:\\Program Files\\Microchip\\xc8\\v2.45\\pic\\include\\c99\\inttypes.h" 3
# 1 "E:\\Program Files\\Microchip\\xc8\\v2.45\\pic\\include\\c99\\bits/alltypes.h" 1 3
# 14 "E:\\Program Files\\Microchip\\xc8\\v2.45\\pic\\include\\c99\\inttypes.h" 2 3
...
unsigned char nReservedTx = (0 < (3) ? 0 : (3) - 1);
...
void mcp2515_read_canMsg(const unsigned char buffer_load_addr, volatile unsigned long* id, volatile unsigned char* ext,
                                  volatile unsigned char* rtrBit, volatile unsigned char* len, volatile unsigned char* buf);
void mcp2515_start_transmit(const unsigned char mcp_addr);
unsigned char mcp2515_isTXBufFree(unsigned char* txbuf_n, unsigned char iBuf);
...
unsigned char readMsgBufID(unsigned char status, volatile unsigned long* id, volatile unsigned char* ext, volatile unsigned char* rtrBit, volatile unsigned char* len, volatile unsigned char* buf);
...
unsigned char mcp2515_setCANCTRL_Mode(const unsigned char newmode) {
    ...
    if ((getMode()) == 0x20 && newmode != 0x20) {
        ...
        unsigned char wakeIntEnabled = (mcp2515_readRegister(0x2B) & 0x40);
        if (!wakeIntEnabled) {
...
unsigned char mcp2515_requestNewMode(const unsigned char newmode) {
    ...
    while (1) {
        ...
        unsigned char statReg = mcp2515_readRegister(0x0E);
...
void mcp2515_id_to_buf(const unsigned char ext, const unsigned long id, unsigned char* tbufdata) {
    ...
    if (ext == 1) {
        tbufdata[3] = (unsigned char)(canid & 0xFF);
        tbufdata[2] = (unsigned char)(canid >> 8);
        canid = (uint16_t)(id >> 16);
        tbufdata[1] = (unsigned char)(canid & 0x03);
        tbufdata[1] += (unsigned char)((canid & 0x1C) << 3);
        tbufdata[1] |= 0x08;
        tbufdata[0] = (unsigned char)(canid >> 5);
    } else {
        tbufdata[0] = (unsigned char)(canid >> 3);
        tbufdata[1] = (unsigned char)((canid & 0x07) << 5);
        tbufdata[3] = 0;
        tbufdata[2] = 0;
    }
}
...
void mcp2515_write_canMsg(const unsigned char buffer_sidh_addr, unsigned long id, unsigned char ext, unsigned char rtrBit, unsigned char len,
    ...
    unsigned char tbufdata[4];
    unsigned char dlc = len | (rtrBit ? 0x40 : 0) ;
    unsigned char i;
...
unsigned char mcp2515_getNextFreeTXBuf(unsigned char* txbuf_n) {
    unsigned char status = mcp2515_readStatus() & (0x54);
    unsigned char i;
...
void enableTxInterrupt(unsigned char enable) {
    unsigned char interruptStatus = mcp2515_readRegister(0x2B);
...
# 1316 "mcp2515_can.c"
unsigned char readRxTxStatus(void) {
    unsigned char ret = (mcp2515_readStatus() & ((0xA8) | (0x03)));
    ret = (ret & (0x08) ? 0x04 : 0) |
          (ret & (0x20) ? 0x08 : 0) |
          (ret & (0x80) ? 0x10 : 0) |
          (ret & (0x03));
...
unsigned char checkClearTxStatus(unsigned char* status, unsigned char iTxBuf) {
    ...
    if (iTxBuf < (3)) {
        ...
        ret = 0;
        for (unsigned char i = 0; i < (3) - nReservedTx; i++) {
...
unsigned char checkError(unsigned char* err_ptr) {
    unsigned char eflg = mcp2515_readRegister(0x2D);
    if (err_ptr) {
        *err_ptr = eflg;
    }
